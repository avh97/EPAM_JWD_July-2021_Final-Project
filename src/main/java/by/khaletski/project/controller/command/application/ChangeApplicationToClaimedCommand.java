...
/**
 * This command changes the status of the application to "claimed".
...
public class ChangeApplicationToClaimedCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        try {
            if (applicationService.changeStatus(id, Status.CLAIMED)) {
                session.setAttribute(Attributes.MESSAGE, "Application status has been set to \"Claimed\".");
            } else {
                session.setAttribute(Attributes.MESSAGE, "Application status has not been set to \"Claimed\".");
            }
