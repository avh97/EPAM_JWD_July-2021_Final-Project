...
/**
 * This command changes the status of the conference to "ended".
...
public class ChangeConferenceToEndedCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        try {
            if (conferenceService.changeStatus(id, Status.ENDED)) {
                session.setAttribute(Attributes.MESSAGE, "Conference status has been set to \"Ended\".");
            } else {
                session.setAttribute(Attributes.MESSAGE, "Conference status hasn't been set to \"Ended\".");
            }
