...
/**
 * This command changes user role to "admin".
...
public class ChangeUserToAdminCommand implements Command {
	...
	public Router execute(HttpServletRequest request) {
		...
		try {
			if (userService.changeRole(id, Role.ADMIN)) {
				session.setAttribute(Attributes.MESSAGE, "User role has been set to \"Admin\".");
			} else {
				session.setAttribute(Attributes.MESSAGE, "User role hasn't been set to \"Admin\".");
			}
