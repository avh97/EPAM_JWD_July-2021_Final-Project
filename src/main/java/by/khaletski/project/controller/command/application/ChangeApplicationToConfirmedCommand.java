...
/**
 * This command changes the status of the application to "confirmed".
...
public class ChangeApplicationToConfirmedCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        try {
            if (applicationService.changeStatus(id, Status.CONFIRMED)) {
                session.setAttribute(Attributes.MESSAGE, "Application status has been set to \"Confirmed\".");
            } else {
                session.setAttribute(Attributes.MESSAGE, "Application status has not been set to \"Confirmed\".");
            }
