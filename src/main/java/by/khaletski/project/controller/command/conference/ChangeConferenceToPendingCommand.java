...
/**
 * This command changes the status of the conference to "pending".
...
public class ChangeConferenceToPendingCommand implements Command {
	...
	public Router execute(HttpServletRequest request) {
		...
		try {
			if (conferenceService.changeStatus(id, Status.PENDING)) {
				session.setAttribute(Attributes.MESSAGE, "Conference status has been set to \"Pending\".");
			} else {
				session.setAttribute(Attributes.MESSAGE, "Conference status hasn't been set to \"Pending\".");
			}
